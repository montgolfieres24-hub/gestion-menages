...
1. Sur github.com : **New repository** → nom `gestion-menages` → **uploading an existing file** → glisse tout le contenu du zip (le dossier `api/` — qui contient les templates —, plus `requirements.txt` et `vercel.json`).
...
- Le bouton en bas ouvre la page **Messages** : un message texte par employée (uniquement ses propres créneaux, sans les attributions des autres), avec un bouton **Copier le message** pour le coller dans WhatsApp ou SMS, et **Marquer envoyé** pour mettre à jour les statuts. Si une date change ensuite côté Airbnb, le statut redescend à « Assigné » pour te rappeler de renvoyer un message.
...
En cas d'erreur, ouvre **`/sante`** (ex. `https://ton-app.vercel.app/sante`) : la page vérifie chaque variable d'environnement, la présence des templates et l'accès à chacune des 4 tables Airtable, et t'indique précisément ce qui bloque. Les erreurs applicatives affichent aussi une page explicative en français au lieu d'un « Internal Server Error ».
